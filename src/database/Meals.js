var Meals = {
    "PatatesToplari":{
        "image": "00001.png",
        "title": "Patates Topları",
        "ingredients": [
            "6 tane orta boy patates",
            "2 orta boy soğan",
            "1 su bardağı köfteli bulgur",
            "4 diş sarımsak",
            "1 çorba kaşığı salça",
            "Tuz, karabiber, pul biber (İsteğe göre baharatlar eklenebilir)",
            "2 çorba kaşığı sıvı yağ"
        ],
        "preperation": [
            "Patatesler soyulmadan tepsiye koyulur ve fırında 200-250 derece 30 dakika pişirilir.",
            "Sonra kabukları soyulur ve ezilir veya rendelenir.",
            "Bulgurlar kaynamış suyla ıslatılır. Soğan rendelenir ve sarımsaklar ezilir.",
            "Tüm malzemeler bir kaba koyulur ve iyice yoğrulur. Küçük toplar haline getirilir. Servis edilir."
        ]
    },
    "AyvaliKurabiye":{
        "image": "00002.jpeg",
        "title": "Ayvalı Kurabiye",
        "ingredients": [
            "1 paket margarin",
            "1 çay bardağı sıvı yağ",
            "3 çorba kaşığı yoğurt",
            "1,5 çay bardağı şeker",
            "1 paket vanilya",
            "1 paket kabartma tozu",
            "Alabildiği kadar un",
            "2 orta boy ayva",
            "1 tatlı kaşığı tarçın",
            "1 su bardağı kırılmış fındık",
            "Pudra şekeri"
        ],
        "preperation": [
            "Harcı: Ayvalık soyulur rendelenir. Suyunu çekene kadar pişirilir. Ocaktan alınıp ceviz ve tarçın eklenir. Karıştırılır ve soğumaya bırakılır.",
            "Hamuru: Un hariç tüm malzemeler eklenip karıştırılır. Un eklenip yoğrulur. Kıvamı yapışmayacak olmalıdır.",
            "Hamur tezgahta açılır ve üçgen şeklinde kesilir ve harç konulup sarılır.",
            "Tepsiye dizilir ve 250 derecede ısıtılmış fırında 40 dakika pembeleşinceye kadar pişirilir. Pudra şekeri ile servis edilir."
        ]
    },
    "OrdekKurabiyesi":{
        "image": "00003.jpeg",
        "title": "Ördek Kurabiyesi",
        "ingredients": [
            "2 adet yumurta",
            "1 adet portakal",
            "Yarım çay bardağı sıvı yağ",
            "Yarım paket / 125 gr margarin",
            "1. 5 şu bardağı şeker",
            "Alabildiği kadar un",
            "1 paket kabartma tozu",
            "Çörek otu"
        ],
        "preperation": [
            "Hamur Yapılışı: Portakalın suyu çıkartılır. Yaklaşık 1 çay bardağı olmalıdır.",
            "İsterseniz süzebilirsiniz ama öyle daha hoş olacaktır.",
            "Margarin, sıvı yağ, portakal suyu, yumurta, şeker eklenir ve karıştırılır.",
            "Un ve kabartma tozu eklenip iyice yoğrulur.",
            "Hamura şekil verilir ve yağlanmış veya pişirme kağıdı konulmuş tepsiye dizilir.",
            "Önceden ısıtılmış fırında 250 derecede 30 dakika pişirilir.",
            "Şekil Verme: Ceviz büyüklüğünde hamurdan parçalar kesilir. Top şekli verilir ve yuvarlanır.",
            "İp bağlarmış gibi hamur parçası iki ucundan bağlanır ve bir ucu arkaya bir ucu öne getirilir.",
            "Arka taraftaki uca çatal ile şekil verilir. Ön taraftaki ucun ucu ezilir.",
            "Çörek otu şekildeki gibi hamurun üstüne konulur."
        ]
    },
    "BardaktaYasPasta":{
        "image": "00005.jpeg",
        "title": "Bardakta Yaş Pasta",
        "ingredients": [
            "1 pandispanya keki",
            "1 adet muz",
            "1 adet nar",
            "Süsleme şekerleri",
            "2 paket krem şanti",
            "1 paket krema"
        ],
        "preperation": [
            "Kek, krema pişirilir. Krem şanti ve meyveler hazırlanır. Bardakla kek bölünür.",
            "Eğer kalınsa kek ikiye bölünüp kesilir. Sırasıyla krema, meyve, kek, krema, meyve, kek koyulur.",
            "Üstüne krem şanti sıkılır ve süsleme şekeri ile meyvelerle süslenir.",
            "Bu tarif yaş pastayı yapmayı beceremeyenler için çok pratik. İsteyen herkes kolaylıkla yapabilir. Afiyet olsun."
        ]
    },
    "YilbasinaOzelSepetler":{
        "image": "00006.jpeg",
        "title": "Yılbaşına Özel Sepetler",
        "ingredients": [
            "2 paket petibor bisküvi",
            "1 yumurta",
            "4 yemek kaşığı kakao",
            "1 bardak süt",
            "1 tepeleme tereyağ",
            "5 yemek kaşığı şeker",
            "Hindistan cevizi",
            "1 çay bardağı kırılmış ceviz",
            "Tam fındık"
        ],
        "preperation": [
            "Bisküviler toz olana kadar ezilir veya rondoya vurulur.",
            "Sonra bir kaba konulup içine kakao eklenir ve karıştırılır. Eğer taneler kalmışsa ezilmemistir.",
            "Sonra içine şeker eklenip karıştırılır. Yumurta ve tereyağ eklenip yorulmaya başlanır. Süt eklenir ve iyice yoğrulur. çevikler eklenip karıştırılır.",
            "Hamurdan küçük bir parça alınıp yuvarlanır. Hindistan cevizin bulanır. İçine delik açılıp fındık yerleştirilir ve üzerine hamurdan yaptığınız saplar eklenir. Dondurulur.",
            "İsteğe göre direkt yuvarlanmış halde de yenebilir."
        ]
    },
    "KuruVeYasNaneCayı":{
        "image": "00007.jpeg",
        "title": "KuruVeYasNaneCayı",
        "ingredients": [
            "1 demet yaş nane veya 1 tatlı kaşığı kuru nane",
            "4 çay bardağı kaynamış su",
            "1 dilim limon",
            "Her 1 bardak su için 1 tatlı kaşığı şeker"
        ],
        "preperation": [
            "Kaynamış suyu deliğe dökülür ve nane içine yaş veya kuru konur.",
            "Yarım saat kısık ateşte demleyin.",
            "Demlendikten sonra bardaklar doldurulur ve isteğe göre bardağa limon sıkılır.",
            "Limon eklenirse tadı daha iyi olur.",
            "Püf Noktaları:",
            "Kuru nane demlenince kırmızı, yaş nanenin demlenmiş hali yeşil olur.",
            "Griple iyi gelir.",
            "Çay ne kar demlenir ise renk o kadar kararır."
        ]
    },
    "MandalinaliLokum":{
        "image": "00008.jpeg",
        "title": "Mandalinalı Lokum",
        "ingredients": [
            "1 kg mandalina  2 su bardağı suyu elde edilir",
            "1 su bardağı şeker",
            "5 kaşık nişasta",
            "Yarım yemek kaşık tereyağı",
            "1 paket vanilya",
            "Hindisdan cevizi",
            "Yarım su bardağı su"
        ],
        "preperation": [
            "Mandalinalar meyve sıkacağına alıp suları sıkılır.",
            "Süzgeçte süzülür .",
            "Tüm malzemeler tencereye koyulur.",
            "Kısık ateşte kaynayana kadar karıştırılır.",
            "Kaynayınca 5 dakika pişirilir.",
            "Borcam tepsi ıslatılıp dökülür.",
            "Buharı çıkınca üzeri streç filmiyle kapatılır yoksa beyaz bir poşetle kapatılıp ağzı bağlanır.",
            "Buzlukta 4 saat bekletilir buzluktan alınıp dilimlenir .",
            "Hindisdan cevizine bulanıp servis yapılır."
        ]
    },
    "PatatesliPeynirliKrep":{
        "image": "00011.jpeg",
        "title": "Patatesli Peynirli Krep",
        "ingredients": [
            "3 yumurta",
            "1 paket kabartma tozu",
            "Alabildiği kadar un",
            "2 su bardağı süt",
            "3 yemek kaşığı sıvı yağ",
            "Yarım kase peynir (Kaşar rendelenmiş olsun)",
            "3 tane haşlanmış patates",
            "1 çay kaşığı tuz",
            "Yarım demet maydanoz",
            "Yarım çay kaşığı kara biber",
            "Yarım çay kaşığı isot",
            "Yarım çay kaşığı kimyon",
            "Yarım kasım domates salçası (Salça tuzlu ise tuz oranını azaltın."
        ],
        "preperation": [
            "Patatesler rendelenir veya ezilir.",
            "Maydanozlar doğranır.",
            "Un, kabartma tozu ve patates hariç tüm malzemeler karıştırılır.",
            "Kalan malzemeler eklenir ve çırpılır.",
            "Yağla kızdırılmış tavaya 1 kepçe hamurdan koyulur ve kaşık yardomi ile tavaya yayılır.",
            "Alt üst pişirilir. İçinde patates olduğundan dikkatlice çevirinin."
        ]
    },
    "KabakCekirdegiHelva":{
        "image": "00012.jpeg",
        "title": "Kabak Çekirdeği Helva",
        "ingredients": [
            "2 su bardağı un ( mısır ve öğütülmüş ve kavrulmuş kabak çekirdeği )",
            "Yarım su bardağı şeker",
            "Yarım su bardağı kaynamış su",
            "2 yemek kaşığı tereyağı",
            "Hindistan cevizi",
            "Pudra şekeri"
        ],
        "preperation": [
            "Tereyağı eritilir ve un eklenip 1-2 dakika kavrulur. Şeker ilave edilip karıştırılır.",
            "Kaynamış su eklenip 5-10 dakika pişirilir.",
            "Şekildeki gibi yuvarlanır veya kare olarak kesilir.",
            "Üstü hindistan cevizi ile kaplanır veya pudra şekeri elenir."
        ]
    }
}

export {Meals};